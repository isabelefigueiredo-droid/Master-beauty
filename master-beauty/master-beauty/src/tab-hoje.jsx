...
function useGoogleData(fetchFn, fallback, connected) {
  const [data, setData] = useState(fallback);
  useEffect(() => {
    if (!connected) { setData(fallback); return; }
    let active = true;
    fetchFn().then((d) => { if (active) setData(d); }).catch(() => {});
    return () => { active = false; };
...
export function MeetingHero({ now, onDebrief, onJoin, agenda }) {
  const ag = agenda || MHB.agenda;
  const toMin = (t) => { const p = String(t || "00:00").split(":"); return +p[0] * 60 + +(p[1] || 0); };
  const nowMin = toMin(now);
  const next = ag.find((e) => e.kind === "meet" && toMin(e.time) >= nowMin) || ag[ag.length - 1] || { time: "--:--", end: "--:--", title: "Sem reuniões hoje", tag: "", people: 0, hot: false, kind: "event" };
  ...
  return (
    <div className="card" style={{ overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: 18, flexWrap: "wrap" }}>
          <div style={{ flex: 1, minWidth: 220 }}>
            ...
            <div style={{ marginTop: 8, fontSize: 13, color: "var(--text-3)", fontWeight: 500 }}>
              {Array.isArray(next.people) ? next.people.length : (next.people || 0)} participantes · começa em {Math.max(0, toMin(next.time) - nowMin)} min
            </div>
          </div>
      ...
      <div style={{ padding: "16px var(--pad) var(--pad)" }}>
        ...
        {ag.length === 0 && <div style={{ color: "var(--text-4)", fontSize: 13.5, padding: "8px 4px" }}>Nenhum evento restante.</div>}
        {ag.map((e) => {
...
/* ── Inbox ──────────────────────────────────────────────── */
export function InboxCard({ onAsk, emails: emailsProp }) {
  const emails = emailsProp ?? MHB.emails.filter((e) => e.needsReply);
  return (
    <div className="card">
      <div className="head">
...
/* ── Drive recentes ─────────────────────────────────────── */
function driveColor(type) {
  if (type === "sheet" || type === "Planilha")       return "#1F8A5B";
  if (type === "slides" || type === "Apresentação")  return "#F9A825";
  if (type === "doc" || type === "Documento")        return "#1B5BD9";
  if (type === "PDF")                                return "#E53E3E";
  return "#888";
}
function driveLabel(type) {
  if (type === "sheet")  return "Planilha";
  if (type === "slides") return "Apresentação";
  if (type === "doc")    return "Documento";
  return type;
}

export function DriveCard({ files: filesProp }) {
  const files = filesProp ?? MHB.drive;
  ...
  return (
    <div className="card">
      ...
      <div className="body" style={{ display: "flex", gap: 12, overflowX: "auto", paddingBottom: 18 }}>
        {files.map((f) => {
          const col = driveColor(f.type);
          const lbl = driveLabel(f.type);
          return (
            <div key={f.id} onClick={() => f.link ? window.open(f.link, "_blank", "noopener") : openDrive()}
              ...
              <div style={{ width: 32, height: 32, borderRadius: 8, display: "grid", placeItems: "center", background: col + "22", color: col, marginBottom: 10 }}><I.file size={17} /></div>
              ...
              <div style={{ fontSize: 11, color: "var(--text-4)" }}>{lbl} · {f.when}</div>
            </div>
          );
        })}
      </div>
...
export default function TabHoje({ onAsk, onDebrief, onJoin, onCreate, googleConnected, onGoogleSignIn, onGoogleSignOut }) {
  ...
  const nMeetPM = agenda.filter((e) => e.kind === "meet" && toMin(e.time) >= 720).length;

  return (
    <>
      {onGoogleSignIn && !googleConnected && (
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 16px", marginBottom: 16, borderRadius: 12, background: "var(--surface)", border: "1px solid var(--bd)", fontSize: 13.5 }}>
          <span style={{ color: "var(--text-3)" }}><I.drive size={16} /></span>
          <span style={{ color: "var(--text-2)", flex: 1 }}>Conecte o Google para ver Gmail e Agenda reais.</span>
          <button onClick={onGoogleSignIn} style={{ height: 34, padding: "0 14px", borderRadius: 8, background: "var(--accent)", color: "var(--on-accent)", fontWeight: 700, fontSize: 13 }}>Conectar</button>
        </div>
      )}
      {googleConnected && onGoogleSignOut && (
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "7px 14px", marginBottom: 14, borderRadius: 10, background: "var(--surface)", border: "1px solid var(--bd)", fontSize: 12.5 }}>
          <span style={{ width: 8, height: 8, borderRadius: 999, background: "var(--ok)", flex: "0 0 auto" }} />
          <span style={{ color: "var(--text-3)", flex: 1 }}>Google conectado — dados em tempo real</span>
          <button onClick={onGoogleSignOut} style={{ fontSize: 12, color: "var(--text-4)", fontWeight: 600 }}>Desconectar</button>
        </div>
      )}
      ...
      <div className="grid">
        <div className="col">
          <MeetingHero now={now} agenda={agenda} onDebrief={(m) => onDebrief({ ...m, mode: "prep" })} onJoin={onJoin} />
          <InboxCard onAsk={onAsk} emails={inboxEmails} />
          <DriveCard files={drive} />
        </div>
        <div className="col">
